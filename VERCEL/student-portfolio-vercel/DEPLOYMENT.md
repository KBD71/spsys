...
### 문제 3: 과제 목록이 비어있음 또는 기록/알림 조회 오류
...
"공개" 시트는 다음 5개 컬럼으로 구성되어야 합니다:

| 컬럼명 | 타입 | 설명 |
|--------|------|------|
| 과제공개 | 체크박스 | 학생이 과제를 볼 수 있는지 (TRUE/FALSE) |
| 대상시트 | 텍스트 | 과제 시트 이름 (예: "수학과제1") |
| 대상반 | 텍스트 | 대상 반 (예: "101, 106" 또는 "전체") |
| 의견공개 | 체크박스 | 교사 코멘트 공개 여부 (TRUE/FALSE) |
| 알림메시지 | 텍스트 | 학생에게 보여줄 알림 텍스트 |

**2. 컬럼 의미 및 사용법**

- **과제공개 = TRUE**: 학생이 과제 목록에서 해당 과제를 볼 수 있음
- **의견공개 = TRUE**: 학생이 "📊 내 기록 (교사 코멘트)"에서 교사의 평가를 볼 수 있음
- **의견공개 = FALSE + 알림메시지 입력**: 학생이 "📢 알림"에서 메시지를 볼 수 있음
- **대상반**: 학생 학번 앞 3자리로 필터링 (예: 학번 10101 → 대상반 "101")

**3. 예시 설정**

| 과제공개 | 대상시트 | 대상반 | 의견공개 | 알림메시지 |
|---------|---------|--------|---------|-----------|
| TRUE | 수학과제1 | 101, 106 | TRUE | (비워둠) |
| TRUE | 영어과제1 | 전체 | FALSE | 곧 채점 결과가 공개됩니다. |
| TRUE | 물리과제1 | 101 | TRUE | (비워둠) |

**4. v1 구조에서 v2로 업그레이드**

Google Apps Script에서 `upgradePublicSheet()` 함수 실행:
1. Google Sheets 열기
2. 상단 메뉴: **도구 > 스크립트 편집기**
3. `PublicSettings.gs` 파일 열기
4. `upgradePublicSheet()` 함수 실행
5. 기존 데이터 보존하며 자동 업그레이드

**5. 추가 확인 사항**
- "과제설정" 시트의 날짜 범위 확인 (시작일/마감일)
- "대상반" 값이 학생 학번 앞 3자리와 일치하는지 확인
